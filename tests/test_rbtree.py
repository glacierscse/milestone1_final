from tsbtreedb.rbtree import RedBlackNode, RedBlackNodeRef, RedBlackTree, ValueRef, Color
...
def test_tree_get_smaller_than():
    t = RedBlackTree(StubStorage())
    t.set(2,1)
    t.set(7,1)
    t.set(3,1)
    ...
    t.set(15,1)
    t.set(16,1)
    assert sorted(t.get_smaller_than(16)) == sorted(list(zip(range(1, 16), [1]*15)))
    assert t.get_smaller_than(1) == [] 
